...
import WEUSD_OPERATIONSABI from '@/utils/weusd_operationsABI.json'
...
export default function Section () {
  ...
  const [inputWEUSD, setInputWEUSD] = useState('')
  ...
  const getWalletTokenBalance = async () => {
    ...
    setTokenBalance({
      ...
      pipi: bigDiv(pipiRes, 10 ** 6),
      usdt: bigDiv(usdtRes, 10 ** 6),
    })
  ...
  useEffect(() => {
    if (connected && PIPIAddr && WEUSDAddr) {
      getWalletTokenBalance()
    }
  }, [connected, PIPIAddr, WEUSDAddr])
  ...
  ">
    <div className="container xl:max-w-[1100px] mx-auto w-full flex-grow">
      ...
      <Mint debouncedHandleChangeAmount={handleChangeAmount} inputWEUSD={inputWEUSD} mintCost={mintCost} handleMINT={handleMINT} handleREDEEM={handleREDEEM} redeemCost={redeemCost} WEUSDPrice={WEUSDPrice} />
      <Balance tokenBalance={tokenBalance} />
    </div>

    <div className='w-full bg-white text-black
      rounded-t-[30px] pt-15 pb-10 mt-12
      md:rounded-t-[50px] md:pt-11 md:pb-15 md:mt-16
      lg:rounded-t-[70px] lg:pt-12 lg:pb-20 lg:mt-18
      xl:rounded-t-[80px] xl:pt-14 xl:pb-25 xl:mt-20
      2xl:rounded-t-[90px] 2xl:pt-15 2xl:pb-30 2xl:mt-[90px]
    '>
...
function Mint ({ debouncedHandleChangeAmount, inputWEUSD, mintCost, handleMINT, handleREDEEM, redeemCost, WEUSDPrice }) {
  const [type, setType] = useState('mint')
  const [amount, setAmount] = useState(0)

  return <div className='w-full bg-white overflow-hidden text-black
    max-w-[630px] mx-auto
    rounded-[15px] mt-7.5 px-3 py-6
    md:rounded-[18px] md:mt-10 md:px-3.5 md:py-7
    lg:rounded-[22px] lg:mt-14 lg:px-4 lg:py-8
    xl:rounded-[26px] xl:mt-16 xl:px-5 xl:py-10
    2xl:rounded-[30px] 2xl:mt-18 2xl:px-6 2xl:py-11
  '>

    <div className='w-full flex items-center justify-center
      gap-6 text-xl h-10
      md:gap-3 md:text-2xl md:h-14
      lg:text-[26px] lg:h-16
      xl:text-[28px] xl:h-18
      2xl:text-3xl 2xl:h-20
    '>
      <button className={cn('flex-1 h-full rounded-full flex items-center justify-center uppercase transition-all duration-300',
        type === 'mint' ? 'text-[#FFE450] bg-black' : 'text-black bg-[#EAEAEA]',
      )} onClick={() => setType('mint')}>
        mint
      </button>
      <button className={cn('flex-1 h-full  rounded-full flex items-center justify-center uppercase transition-all duration-300',
        type === 'redeem' ? 'text-[#FFE450] bg-black' : 'text-black bg-[#EAEAEA]',
    ...
    '>
      ...
      '>
        ...
        <span>
          {type === 'mint' ? mintCost.usdt : redeemCost.usdt} USDC
        </span>
      </div>
    ...
    <button onClick={() => { type == 'mint' ? handleMINT() : handleREDEEM() }} className='w-full  flex items-center justify-center rounded-full relative
      bg-black text-[#FFE450] hover:text-[#FFE450] hover:bg-black transition-all duration-300
      mt-6 h-10 text-lg
      md:mt-8 md:h-14 md:text-xl
      lg:mt-12 lg:h-16 lg:text-2xl
      xl:mt-14 xl:h-18 xl:text-[26px]
      2xl:mt-15 2xl:h-20 2xl:text-[28px]
    '>
      <span>
        {type === 'mint' ? 'Mint WEUSD & PIPI' : 'Redeem USDC & MOVE'}
      </span>
...
function Pool ({ poolAmount, movePrice }) {
  const infoList = [
    ...
    { id: 3, title: 'Omnichain Ready', intro: 'Designed with cross-chain functionality in mind, $WEUSD lets you move assets smoothly between multiple networks.' },
  ]
  ...
  '>
    ...
    '>
      <table className='w-full overflow-hidden'>
        ...
        <tbody className='bg-white font-open-sans
          text-[10px]
          md:text-xs
          lg:text-sm
          xl:text-base
